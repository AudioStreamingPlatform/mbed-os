...
void USBPhyHw::deinit()
{
    ...
    [[maybe_unused]] ret_code_t ret = nrf_drv_usbd_uninit();
    MBED_ASSERT(ret == NRF_SUCCESS);
...
void USBPhyHw::ep0_read(uint8_t *data, uint32_t size)
{
    ...
    [[maybe_unused]] ret_code_t ret = nrf_drv_usbd_ep_transfer(NRF_DRV_USBD_EPOUT0, transfer);
    MBED_ASSERT(ret == NRF_SUCCESS);
}
...
void USBPhyHw::ep0_write(uint8_t *buffer, uint32_t size)
{
    ...
    [[maybe_unused]] ret_code_t ret = nrf_drv_usbd_ep_transfer(NRF_DRV_USBD_EPIN0, transfer);
    MBED_ASSERT(ret == NRF_SUCCESS);
}
